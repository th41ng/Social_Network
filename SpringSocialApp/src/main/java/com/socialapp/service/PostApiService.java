...
public interface PostApiService {
     ...
     * Requires the current authenticated user for permission checks during update
     * and for associating the user with a new post.
     *
     * @param postFromRequest The post data from the request. For updates, this should include postId.
     * @param currentUser The currently authenticated user performing the action.
     * @return The saved or updated PostDTO object.
     * @throws SecurityException if the user is not authorized to update the post.
     * @throws jakarta.persistence.EntityNotFoundException if the post to be updated is not found.
     */
    PostDTO addOrUpdatePost(Post postFromRequest, User currentUser); // <<<< THAY ĐỔI SIGNATURE

    /**
     * Deletes a post by its ID for the API.
     * Requires the current authenticated user for permission checks.
     *
     * @param postId The ID of the post to delete.
     * @param currentUser The currently authenticated user performing the action.
     * @return true if deletion was successful (post was found and user had permission).
     * (Hoặc có thể là void và ném Exception nếu thất bại)
     * @throws SecurityException if the user is not authorized to delete the post.
     * @throws jakarta.persistence.EntityNotFoundException if the post to be deleted is not found.
     */
    boolean deletePost(int postId, User currentUser); // <<<< THAY ĐỔI SIGNATURE

    /**
     * Gets comments for a post by its ID for the API.
     * Only returns comments for non-deleted posts and non-deleted comments.
     *
