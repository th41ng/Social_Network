...
const Home = () => {
    ...
    const loadPosts = async () => {
        try {
            setLoading(true);
            const url = `${endpoints["posts"]}?page=${page}`;
            const res = await Apis.get(url);

            setPosts((prevPosts) => {
                if (res.data.length === 0) {
                    setPage(0); // Stop pagination if no more posts
                    return prevPosts;
                }
                return page === 1 ? res.data : [...prevPosts, ...res.data];
            });
        } catch (error) {
    ...
    const handleReactionClick = async (postId, reactionType) => {
        try {
            ...
            setPosts((prevPosts) =>
                prevPosts.map((post) =>
                    post.postId === postId ? { ...post, reactions: updatedReactions } : post
                )
            );
        } catch (error) {
            console.error("Error updating reaction:", error);
    ...
    useEffect(() => {
        if (page > 0 && isLoggedIn) loadPosts();
    }, [page, q, isLoggedIn]);

    const loadMore = () => {
        if (!loading && page > 0) setPage((prevPage) => prevPage + 1);
    };
                        ...
                        <Card key={post.postId} className="mb-3 shadow-sm rounded">
                            <Card.Body>
                                <div className="d-flex align-items-center mb-3">
                                    ...
                                    />
                                    <div>
                                        <strong>{post.userFullName || "Ẩn danh"}</strong>
                                        <br />
